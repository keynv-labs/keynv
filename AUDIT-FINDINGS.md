...
| H4  | HIGH     | RESOLVED | "fix: close audit HIGHs (H1, H4, H5)" | Bootstrap password reads from `KEYNV_BOOTSTRAP_OWNER_PASSWORD` env / stdin / argv-with-`--unsafe-allow-argv`. Legacy `KEYNV_BOOTSTRAP_PASSWORD` is accepted with a deprecation warning. argv default is refused. |
