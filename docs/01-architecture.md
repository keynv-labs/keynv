...
| SQLite WAL corruption | Litestream restore from S3 / B2 (RPO ~1s). Documented runbook in `docs/phases/05-hardening-and-oss-release.md`. |
...
- **Why not Vault?** Excellent product, wrong threat model focus. Vault has no agent-isolation story, no per-agent installers, no streaming output redactor.
