...
> No findings raised yet — this scaffold lists every threat row so we don't
> miss any during the walkthrough.
...
| --- | --- | --- |
| Spam `keynv exec` to exhaust subprocess slots | **NOT IMPLEMENTED** in Phase 1-4. The threat model says "rate limit per user/agent: 100 exec/min default, configurable" but no rate limiter exists. | 🔴 **Finding A1 (Medium)** — see below |
...
### 🔴 Finding A1 (Medium): No rate limiter on `keynv exec` (or any auth'd endpoint)

**Where**: `apps/server/src/app.ts` — Hono app has no rate-limit middleware. Every authenticated endpoint accepts unlimited requests per user / IP / token.

**Impact**: A compromised CLI token or a misbehaving agent loop can hammer the server with `secret.read` or `keynv exec` calls. The audit chain absorbs the cost (each request appends a row), but the SQLite single-writer can become the bottleneck on a busy small server.

**Remediation**: Add `hono-rate-limiter` (or roll a small token-bucket against a Map keyed by user_id). Default: 100 requests/min per user, configurable via env. Apply globally to authed routes.

**Severity reasoning**: Medium because (a) it's not a confidentiality/integrity issue — RBAC still holds; (b) the affected pattern is "noisy neighbour" not "data exfiltration"; (c) self-hosters typically run behind a proxy that already rate-limits.

**Fix decision**: Defer to Stream C release-pipeline window (small change, but easier to add when there's a real rate-limit story including the `error_code: rate_limited` response pattern that's already declared in `lib/errors.ts`).
...
- **AF-6** — Implement rate limiter (Finding A1 above).
