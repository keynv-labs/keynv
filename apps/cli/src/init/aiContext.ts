/**
 * AI-context markdown that `keynv init` writes into a project's
 ...
 *
 * The block is wrapped in HTML-comment markers so re-running
 * `keynv init` updates only this section without touching the
...
export const KEYNV_BLOCK_START = '<!-- keynv:start -- generated by `keynv init`; safe to leave intact, will be refreshed on re-run -->';
